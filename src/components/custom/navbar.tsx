        ...
        <div className="flex items-center gap-12">
          ...
          <div className="hidden lg:flex gap-12">
            {navLinks.map((link, index) =>
              link.isHome ? (
                <ScrollIntoView selector={"#" + link.route} key={index}>
                  <Link className="font-medium hover:bg-gradient-to-r from-[#7e02b7] via-purple-700 hover:text-transparent bg-clip-text to-[#c240ff] bg-300% animate-gradient" href={"/"}>{link.name}</Link>
                </ScrollIntoView>
              ) : (
                <Link
                  href={"/" + link.route}
